...
:::note
macOSおよびLinuxユーザーは、外部のUSBドライバーをインストールする必要はありません。Platform Toolsをインストールすれば、ADBおよびFastbootはすぐに機能します。
:::
...
:::warning
開始する前にブートローダーがアンロックされていることを確認してください。まだアンロックしていない場合は、[ブートローダーのアンロック](#ブートローダーのアンロック)ガイドを参照してください。
:::

:::danger 免責事項および警告
- **一般的なガイド:** これは大半のシナリオで動作するように設計された一般的なガイドです。常にROMの開発者またはメンテナーが提供する具体的な指示を相互参照し、それに従ってください。
- **データ損失:** 初めてカスタムROMをフラッシュする場合、またはクリーンフラッシュを実行する場合、**すべてのユーザーデータが消去されます**。続行する前にデータをバックアップしてください（例: Google Oneバックアップ、または `adb pull` やFTPを介して手動でフォルダーをコピーする、あるいはすでにルート化されている場合はSwift Backupなどのルートバックアップソリューションを使用するなど）。
- **ドライバーとツールのセットアップ:** アンロックされたブートローダーと、PC上に適切に設定されたUSB、ADB、Fastbootドライバーが必要です。
- **デバイスの安全性:** カスタムROMのフラッシュや実験的ビルドの使用は、デバイスのブートループやブリック（文鎮化）のリスクを伴います。ご自身の責任において行ってください。プロジェクトの作成者および貢献者は、デバイスへのいかなる損害に対しても責任を負いません。
:::

:::info
- **クリーンフラッシュ**はすべてのユーザーデータを消去し、カスタムROMを初めてフラッシュする場合や別のROMに切り替える場合に必須となります。
- **ダーティフラッシュ**（データを消去しないアップデート）は、一般的に同じROMのマイナーアップデートのみでサポートされます。メジャーバージョンのアップグレード（例：Nothing OSやAndroidのバージョン移行）では通常サポートされません。
- **Fastboot用 vs. Recovery用 ROM:** パッケージの内容を確認することで、両者を区別できます。
  - zipに `payload.bin` ファイルが含まれている場合、それは**Recovery/Sideload**ベースのROM（またはOTA zip）です。
  - 複数の個別のパーティション `.img` ファイルが含まれている場合、それは**Fastboot**ベースのROMです。
:::

<br />

#### Recovery / SideloadベースのROM

:::note 重要な注意事項
- **サイドロードの進行状況とPCのステータス:** `adb sideload` を実行中、PC上の進行状況インジケーターはしばしば **47%** 付近で一時停止し、`Total xfer: 1.00x` または `adb: failed to read command: Success`, `No error`, `Undefined error: 0` などのエラーメッセージが表示されることがあります。これは正常な動作であり、転送が成功したことを示しています。常に携帯電話の画面を参照してインストールが完了したことを確認してください（`exit status 0` を探します）。PC側で47%で止まることや、フォーマット後にデバイス画面に `/metadata/ota` エラーが表示されることは予期された動作です。
- **カスタムリカバリ経由のフラッシュ:** TWRPやOrangeFoxなどのカスタムリカバリを使用して、Recoveryベースのビルド（`ROM.zip`）を直接フラッシュすることは通常可能です。ただし、機能が壊れていないかデベロッパーノートを確認してください。これを行うと、自動OTAアップデートが無効になる可能性があることに注意してください。OTAは通常、ROMに同梱されている純正リカバリイメージに依存しているためです。
- **エラー7 (kInstallDeviceOpenError):** 純正ROMから切り替える場合、または別のカスタムROMから切り替える場合に `Error Applying update: 7 (ErrorCode: kInstallDeviceOpenError)` が発生した（またはリカバリ経由の直接フラッシュが失敗した）場合、`super_empty.img` パーティションイメージをフラッシュする必要があります。このファイルは、特定のデバイスモデル用のTelegramディスカッショングループから入手できます。
- **サイドロード失敗のトラブルシューティング:** サイドロードが失敗し続ける場合は、まず利用可能なROMのFastbootバージョンをフラッシュしてデバイスをセットアップします。その後、目的のAOSP/CLO ROMに対応するリカバリイメージをフラッシュし、それを介してROM zipをサイドロードします。これにより正常に完了するはずです。
- **パーティションイメージの不足:** メンテナーがリリースアセットに個別の `boot`、`recovery`、または `vendor_boot` パーティションイメージを提供していない場合があります。その場合、[otaripper](https://github.com/syedinsaf/otaripper) などのツールを使用して、ROMのzipファイル内の `payload.bin` からこれらのイメージを抽出できます。
:::

##### クリーンフラッシュ

A. **前提条件**
- **開発者向けオプションとUSBデバッグ**が有効で、**ブートローダーがアンロック**されていること。
- **ADB & Fastbootを備えたPC**（[Platform Tools](#platform-tools-adb--fastboot)を参照）。
- 必要なファイルがダウンロードされていること（一部のパーティションイメージはデバイスモデルによって異なる場合があります）：
  - `boot.img`
  - `vendor_boot.img`
  - `recovery.img`
  - `super_empty.img` *（純正ROMからフラッシュする場合のみ必要。他のカスタムROMから移行する場合はスキップ）*
  - `rom.zip`
  - `GAppsパッケージ` *（オプション、"Vanilla"ビルドのみ）*

:::tip
ダウンロードしたすべての `.img` ファイルを `platform-tools` フォルダ内に直接配置すると、ターミナルコマンドを簡略化できます。そうしない場合は、フラッシュコマンドの実行時に各イメージのフルファイルパスをターミナルにドラッグアンドドロップする必要があります。
:::

B. **ブートローダーに再起動する**
1. 電話をブートローダーモードに再起動します：
   ```sh
   adb reboot bootloader
   ```
2. デバイスが認識されていることを確認します：
   ```sh
   fastboot devices
   ```
   :::note
   デバイスが `<serial> fastboot` として表示されている必要があります。何も表示されない場合は、[USBドライバー](#usbドライバー)ガイドを参照して接続を確認するか、USBドライバーを更新してください。
   :::
...
E. **Superパーティションの消去（純正 → カスタム移行時のみ）**
:::warning
- 他のカスタムROMから移行する場合は、このステップをスキップしてください。
- 現在カスタムROMのfastbootdインターフェース画面にいない場合は、**このコマンドを実行しないでください**。
- このコマンドを実行した場合は、システムROM全体のフラッシュが正常に完了するまで**デバイスを再起動しないでください**。
:::
空のスーパーイメージを使用してsuperパーティションを消去します：
```sh
fastboot wipe-super super_empty.img
```

F. **リカバリに再起動する**
新しくフラッシュしたリカバリに起動します：
```sh
...
H. **ROMをサイドロードする**
1. リカバリメニューで、**Apply Update** → **Apply from ADB** を選択します。
2. PC上でADBサイドロード接続を確認します：
   ```sh
   adb devices
   ```
   *期待される出力:* `<serial>   sideload`
   :::note
   デバイスが検出されない、または `unauthorized`（未許可）と表示される場合：
   - USBケーブルを抜き差しします。
   - USBドライバーをインストールまたは更新します（[USBドライバー](#usbドライバー)を参照）。
   :::
3. サイドロードインストールを開始します：
   ```sh
   adb sideload rom.zip
   ```
   :::note
   ターミナルの進行状況バーが **47%** で一時停止し、`Total xfer: 1.00x` と表示されることがありますが、これは正常な動作です。ROMは正常にフラッシュされました。電話の画面を確認して確認してください。
   :::

I. **GAppsおよび追加パッケージ**
サイドロードが完了すると、リカバリから追加のパッケージをインストールするかどうか尋ねられます：
- **Vanillaビルド:** **YES** を選択し、リカバリに再起動してから、GAppsパッケージをサイドロードします：
  ```sh
  adb sideload gapps.zip
  ```
- **GMSビルド:** **NO** を選択します（Google Appsはすでに含まれています）。

J. **最後のワイプと起動**
1. 暗号化をクリアするため、最後にもう一度 **Factory Reset** → **Format data / factory reset** に移動します。
2. **Reboot system now** を選択します。
...
:::info
- Magisk/KernelSUのルートアクセスとモジュールは、通常ダーティフラッシュ後も維持されます。
- ダーティフラッシュのサポートは、**ROMおよびメンテナーによって異なります**。試みる前に必ずリリースノートまたは変更履歴を確認してください。
- メンテナーがダーティフラッシュをサポートしていると明記していない場合は、**クリーンフラッシュを実行する必要があります**。
:::

:::note
- **Nothing Phone (2a) Plus (pacmanpro) ユーザー:** デバイスに Fenrir をフラッシュしている場合、ダーティフラッシュは**サポートされていません**。行うとデバイスがブリックするかブートループを引き起こす可能性があります。ROMを更新または切り替えるたびにクリーンフラッシュが必要です。
- Fenrirをフラッシュしていない場合は、OTAとリカバリサイドロードのどちらのダーティフラッシュ方法も正常に動作します。
:::

A. **方法1: OTAアップデート**
1. **設定** → **システム** → **システムアップデート** に移動しますします。
2. 利用可能な最新のOTAアップデートをダウンロードします。
3. ダウンロードと検証プロセスが完了したら、**再起動**をタップします。
4. デバイスはアップデートをインストールし、自動的に再起動します。

B. **方法2: リカバリサイドロード**
1. デバイスをリカバリモードに再起動します：
   ```sh
   adb reboot recovery
   ```
2. **Apply Update** → **Apply from ADB** に移動します。
3. PC上で接続を確認します：
   ```sh
   adb devices
   ```
   *期待される出力:* `<serial>   sideload`
   :::note
   デバイスが検出されない、または `unauthorized` と表示される場合：
   - USBケーブルを抜き差しします。
   - USBドライバーをインストールまたは更新します（[USBドライバー](#usbドライバー)を参照）。
   :::
4. ROMのzipファイルをサイドロードします：
   ```sh
   adb sideload rom.zip
   ```
5. 追加パッケージのインストールを求められたら、**NO** を選択します（GAppsを再フラッシュする必要がある場合を除く）。
6. **Reboot system now** を選択します。
...
:::note
- **ファームウェア要件:** FastbootベースのROMは、インストール前にベースとなる特定のバージョンの純正Nothing OSファームウェアを必要とすることがよくあります。メンテナーのリリースノートを参照してください。ベースファームウェアイメージは、[ファームウェアデータベース](/docs/firmware)の `image-firmware.7z` アーカイブから取得できます。
:::

A. **前提条件**
- **USBデバッグ**が有効で、**ブートローダーがアンロック**されていること。
- **ADB & Fastbootを備えたPC**（[Platform Tools](#platform-tools-adb--fastboot)を参照）。
- ダウンロードしたFastboot ROMパッケージ（通常、ファイル名に `image` または `fastboot` が含まれますが、これは必須ではありません）。
...
2. デバイスが認識されていることを確認します：
   ...
   ```
   :::note
   デバイスが `<serial> fastboot` としてリストされている必要があります。検出されない場合は、[USBドライバー](#usbドライバー)ガイドを参照してください。
   :::

C. **ROMをフラッシュする**
実行したいフラッシュの種類に応じて、次のコマンドを実行します：

- **クリーンフラッシュの場合（すべてのユーザーデータを消去します）:**
  ```sh
  fastboot -w update <ROMのzipへのパス>
  ```

- **ダーティフラッシュの場合（ユーザーデータを保持します）:**
  ```sh
  fastboot update <ROMのzipへのパス>
  ```
